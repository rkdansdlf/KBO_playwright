...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(LOG_DIR / "backfill.log"), logging.StreamHandler()],
)
                ...
                try:
                    ...
                    if payload:
                        ...
                        if repair_game_parent_from_existing_children(game_id):
                            logger.info(
                                f"✅ [{success_count + fail_count + 1}/{len(missing_ids)}] Repaired {game_id} from existing child rows"
                            )
                            success_count += 1
                            break
                        if reason and "cancelled" in reason.lower():
                            ...
                            if save_game_snapshot(cancel_payload, status=GAME_STATUS_CANCELLED):
                                logger.info(
                                    f"✅ [{success_count + fail_count + 1}/{len(missing_ids)}] Marked {game_id} as CANCELLED"
                                )
                                success_count += 1
                                break
