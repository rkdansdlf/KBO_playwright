...
async def verify_fix(game_id):
    logger.info(
        "[DEBUG] scripts/maintenance/verify_2018_fix.py performs a live parser probe only. It does not persist data."
    )
    logger.info(f"🕵️‍♀️ Verifying fix for {game_id}...")
    crawler = GameDetailCrawler()

    # 1. Crawl
    data = await crawler.crawl_game(game_id, game_id[:8])
    if not data:
        logger.error("❌ Crawl returned None")
        return

    logger.info("✅ Crawl Data Retrieved")
    # Check sample hitter stats
    home_hitters = data["hitters"]["home"]
    sample_hitter = home_hitters[0] if home_hitters else None
    if sample_hitter:
        logger.info(f"   Sample Hitter: {sample_hitter['player_name']}")
        logger.info(f"   Stats: {sample_hitter['stats']}")
        if sample_hitter["stats"].get("runs") is not None:
            logger.info(f"   Runs: {sample_hitter['stats']['runs']} (Expected > 0 for some players)")
        else:
            logger.error("   ❌ 'runs' key missing in stats!")
    ...
    logger.info(f"   Total Home Runs parsed: {total_runs}")

    total_runs_away = sum(h["stats"].get("runs", 0) or 0 for h in data["hitters"]["away"])
    logger.info(f"   Total Away Runs parsed: {total_runs_away}")

    expected_home = data["teams"]["home"]["score"]
    expected_away = data["teams"]["away"]["score"]

    logger.info(f"   Scoreboard: Home={expected_home}, Away={expected_away}")
