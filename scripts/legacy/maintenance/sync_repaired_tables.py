...
def sync_repaired_tables():
    with SessionLocal() as session:
        ...
        # syncer.sync_simple_table(GameBattingStat, ['game_id', 'player_id', 'appearance_seq'], batch_size=10000)

        # logger.info("🚀 Syncing repaired game_pitching_stats to OCI...")
        # from src.models.game import GamePitchingStat
        # syncer.sync_simple_table(GamePitchingStat, ['game_id', 'player_id', 'appearance_seq'], batch_size=10000)

        logger.info("🚀 Syncing repaired game_lineups to OCI...")
        from src.models.game import GameLineup

        synced = syncer.sync_simple_table(GameLineup, ["game_id", "player_id", "appearance_seq"], batch_size=10000)
