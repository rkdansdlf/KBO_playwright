    ...
    with SrcSession() as src, DstSession() as dst:
        ...
        for model in [PlayerSeasonBatting, PlayerSeasonPitching]:
            ...
            for row in rows:
                ...
                exists = (
                    dst.query(model)
                    .filter_by(player_id=row.player_id, season=row.season, league=row.league, level=row.level)
                    .first()
                )
                if not exists:
                    dst.add(model(**data))
        ...
        missing_pitching = (
            src.query(GamePitchingStat)
            .filter(GamePitchingStat.game_id.like("2025%"), GamePitchingStat.team_code.in_(["DB", "KH"]))
            .all()
        )
        ...
        for row in missing_pitching:
            ...
            exists = (
                dst.query(GamePitchingStat)
                .filter_by(game_id=row.game_id, player_id=row.player_id, appearance_seq=row.appearance_seq)
                .first()
            )
            if not exists:
