...
def detect_unresolved():
    ...
    with get_db_session() as session:
        ...
        for name, team, season in sorted(all_unresolved):
            ...
            result = session.execute(
                text("SELECT player_id, name, team, uniform_no FROM player_basic WHERE name = :name"), {"name": name}
            ).fetchall()
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f, fieldnames=["player_name", "team_code", "season", "suggested_player_id", "match_type"]
        )
        writer.writeheader()
