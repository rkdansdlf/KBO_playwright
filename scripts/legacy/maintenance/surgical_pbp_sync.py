    ...
    with OCI_Session() as oci_session, LocalSession() as local_session:
        for year in years:
            ...
            deleted = oci_session.execute(
                text("DELETE FROM game_play_by_play WHERE game_id LIKE :pattern"), {"pattern": pattern}
            )
            oci_session.commit()
            ...
            for offset in range(0, local_count, chunk_size):
                rows = (
                    local_session.query(GamePlayByPlay)
                    .filter(GamePlayByPlay.game_id.like(pattern))
                    .offset(offset)
                    .limit(chunk_size)
                    .all()
                )
