...
def audit_game_stats(year: int = None):
    ...
    stats_results = stats_query.group_by(GameBattingStat.game_id, GameBattingStat.team_side).all()
    ...
    dupe_query = (
        ...
        .group_by(GameBattingStat.game_id, GameBattingStat.player_id, GameBattingStat.player_name)
        .having(func.count("*") > 1)
    )
