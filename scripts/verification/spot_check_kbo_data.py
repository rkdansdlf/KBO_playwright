...
class SpotChecker:
    ...
    async def check_players(self, sample_players: int, year: Optional[int] = None) -> int:
            ...
            for idx, p in enumerate(players, 1):
                ...
                try:
                    ...
                    if not live_profile:
                        ...
                        print(f"   ⚠️ Could not load profile for {p.name} (ID: {pid_str}). (Possibly retired/inactive or layout mismatch)")
                        continue
    ...
    async def check_games(self, sample_games: int, year: Optional[int] = None) -> int:
        ...
        with SessionLocal() as session:
            ...
            for idx, g in enumerate(games, 1):
                ...
                print(f"[{idx}/{len(games)}] Checking Game: {g.game_id} ({g.away_team} @ {g.home_team}) - {game_date_str}")
                ...
                try:
                    ...
                    db_innings = session.query(GameInningScore).filter_by(game_id=g.game_id).order_by(GameInningScore.team_side, GameInningScore.inning).all()
                    ...
                    for side in ("home", "away"):
                        ...
                        if len(db_side_innings) != len(live_line):
                            self.log_mismatch("GameInnings", g.game_id, f"{side}_innings_length", len(db_side_innings), len(live_line), f"DB: {db_side_innings} vs Live: {live_line}")
                        else:
                            for inning_idx, (db_runs, live_runs) in enumerate(zip(db_side_innings, live_line), 1):
                                if db_runs != live_runs:
                                    self.log_mismatch("GameInnings", g.game_id, f"{side}_inning_{inning_idx}", db_runs, live_runs)
                        ...
                        for h_idx, live_hitter in enumerate(live_hitters, 1):
                            ...
                            if key not in db_batting:
                                ...
                                if fallback_matches:
                                    ...
                                    self.log_mismatch("GameBatting", g.game_id, f"missing_hitter_{h_name}", None, f"Seq {h_seq}")
                                    continue
                            else:
                            ...
                            for stat_key in ("at_bats", "hits", "runs", "home_runs", "rbi", "walks", "strikeouts"):
                                ...
                                if db_val != live_val:
                                    self.log_mismatch("GameBatting", g.game_id, f"{h_name}_{stat_key}", db_val, live_val)
                        ...
                        for p_idx, live_pitcher in enumerate(live_pitchers, 1):
                            ...
                            if key not in db_pitching:
                                ...
                                if fallback_matches:
                                    ...
                                    self.log_mismatch("GamePitching", g.game_id, f"missing_pitcher_{p_name}", None, f"Seq {p_seq}")
                                    continue
                            else:
                                db_pitcher = db_pitching[key]
                                
                            live_stats = live_pitcher.get("stats") or {}
                            for stat_key in ("innings_outs", "hits_allowed", "runs_allowed", "earned_runs", "strikeouts", "walks_allowed"):
                                db_val = getattr(db_pitcher, stat_key) or 0
                                live_val = live_stats.get(stat_key) or 0
                                if db_val != live_val:
                                    self.log_mismatch("GamePitching", g.game_id, f"{p_name}_{stat_key}", db_val, live_val)
    ...
    try:
        ...
        if checker.mismatches:
            ...
            for m in checker.mismatches:
                print(f"  - [{m['category']}] {m['id']}: {m['field']} (DB: '{m['db_value']}' vs Live: '{m['live_value']}')")
            ...
            for p_id in remedy_players:
                print(f"    python3 -m src.crawlers.player_profile_crawler --year 2025 --force")
        else:
