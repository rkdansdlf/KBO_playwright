...
class SpotChecker:
    ...
    def _compare_player_profile(self, p, live_profile: dict, pid_str: str) -> None:
        ...
        for db_f, live_f in [("height_cm", "height_cm"), ("weight_kg", "weight_kg")]:
            ...
            if db_val is not None and live_val is not None and abs(db_val - live_val) > 2:
    ...
    async def check_players(self, sample_players: int, year: int | None = None) -> int:
        logger.info("Starting Spot-Check on %s Random Players...", sample_players)
        with SessionLocal() as session:
            ...
            players = (
                ...
                if year
                else query.order_by(func.random()).limit(sample_players).all()
            )
            if not players:
                logger.info("No players found matching search criteria.")
                return 0
            checked_count = 0
            player_errors = (TimeoutError, ConnectionError, OSError, ValueError, KeyError, AttributeError, TypeError)
            for idx, p in enumerate(players, 1):
                pid_str = str(p.player_id)
                logger.info("[%s/%s] Checking Player: %s (ID: %s)", idx, len(players), p.name, pid_str)
                try:
                    live_profile = await self.profile_crawler.crawl_player_profile(pid_str, position=p.position)
                    if not live_profile:
                        logger.info("Could not load profile for %s (ID: %s).", p.name, pid_str)
                        continue
                    checked_count += 1
                    self._compare_player_profile(p, live_profile, pid_str)
                except player_errors as e:
                    logger.info("Error checking player %s (%s): %s", p.name, pid_str, e)
            return checked_count

    def _check_innings(self, game_id: str, g, teams_info: dict, session) -> None:
        db_innings = (
    ...
    def _check_batting(self, game_id: str, live_game: dict, session) -> None:
        db_batting = {
            (b.player_name, b.appearance_seq): b
            for b in session.query(GameBattingStat).filter_by(game_id=game_id).all()
        }
        for side in ("home", "away"):
            ...
            for h_idx, live_hitter in enumerate(live_hitters, 1):
                ...
                if key not in db_batting:
                    ...
                    if fallback_matches:
                        ...
                        self.log_mismatch("GameBatting", game_id, f"missing_hitter_{h_name}", None, f"Seq {h_seq}")
                        continue
                else:
                ...
                for stat_key in ("at_bats", "hits", "runs", "home_runs", "rbi", "walks", "strikeouts"):
                    ...
                    if db_val != live_val:
                        self.log_mismatch("GameBatting", game_id, f"{h_name}_{stat_key}", db_val, live_val)

    def _check_pitching(self, game_id: str, live_game: dict, session) -> None:
        db_pitching = {
            (p.player_name, p.appearance_seq): p
            for p in session.query(GamePitchingStat).filter_by(game_id=game_id).all()
        }
        for side in ("home", "away"):
            ...
            for p_idx, live_pitcher in enumerate(live_pitchers, 1):
                ...
                if key not in db_pitching:
                    ...
                    if fallback_matches:
                        ...
                        self.log_mismatch("GamePitching", game_id, f"missing_pitcher_{p_name}", None, f"Seq {p_seq}")
                        continue
                else:
                ...
                for stat_key in (
                    ...
                    live_val = live_stats.get(stat_key) or 0
                    if db_val != live_val:
                        self.log_mismatch("GamePitching", game_id, f"{p_name}_{stat_key}", db_val, live_val)
    ...
    async def check_games(self, sample_games: int, year: int | None = None) -> int:
        ...
        with SessionLocal() as session:
            ...
            for idx, g in enumerate(games, 1):
                ...
                try:
                    live_game = await self.game_crawler.crawl_game(str(g.game_id), game_date_str)
                    if not live_game:
                    ...
                    if g.home_score != live_home_score:
                        self.log_mismatch("Game", str(g.game_id), "home_score", g.home_score, live_home_score)
                    if g.away_score != live_away_score:
                        self.log_mismatch("Game", str(g.game_id), "away_score", g.away_score, live_away_score)
                    live_stadium = (live_game.get("metadata") or {}).get("stadium")
                    if g.stadium and live_stadium and g.stadium != live_stadium:
                        self.log_mismatch("Game", str(g.game_id), "stadium", g.stadium, live_stadium)
                    self._check_innings(str(g.game_id), g, teams_info, session)
                    self._check_batting(str(g.game_id), live_game, session)
                    self._check_pitching(str(g.game_id), live_game, session)
                except (
                    TimeoutError,
                    ConnectionError,
                    OSError,
