...
async def main():
    ...
    async with async_playwright() as p:
        ...
        page.on(
            "request",
            lambda req: requests_log.append(f">> {req.method} {req.url}") if "koreabaseball" in req.url else None,
        )
        page.on(
            "response",
            lambda res: requests_log.append(f"<< {res.status} {res.url}") if "koreabaseball" in res.url else None,
        )
        ...
        print("=== Game-related window functions ===")
