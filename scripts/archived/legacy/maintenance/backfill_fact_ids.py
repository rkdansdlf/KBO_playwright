...
def backfill_fact_tables(conn):
    ...
    with conn.begin():
        ...
        for code, fid in code_to_fid.items():
            ...
            conn.execute(
                text(
                    f"UPDATE game SET home_franchise_id = {fid} WHERE home_team = '{code}' AND home_franchise_id IS NULL"
                )
            )
            conn.execute(
                text(
                    f"UPDATE game SET away_franchise_id = {fid} WHERE away_team = '{code}' AND away_franchise_id IS NULL"
                )
            )
            conn.execute(
                text(
                    f"UPDATE game SET winning_franchise_id = {fid} WHERE winning_team = '{code}' AND winning_franchise_id IS NULL"
                )
            )
        ...
        for table in fact_tables:
            for code, fid in code_to_fid.items():
                conn.execute(
                    text(f"UPDATE {table} SET franchise_id = {fid} WHERE team_code = '{code}' AND franchise_id IS NULL")
                )
        ...
        for fid, canonical in FRANCHISE_CANONICAL_CODE.items():
            for table in fact_tables:
                conn.execute(
                    text(
                        f"UPDATE {table} SET canonical_team_code = '{canonical}' WHERE franchise_id = {fid} AND canonical_team_code IS NULL"
                    )
                )
