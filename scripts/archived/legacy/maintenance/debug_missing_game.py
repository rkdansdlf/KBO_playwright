...
async def main():
    print(
        "[DEBUG] scripts/maintenance/debug_missing_game.py performs a live parser probe only. It does not persist data."
    )
    ...
    try:
        ...
        if data:
            ...
            print(
                f"Away: {teams.get('away', {}).get('name')} ({teams.get('away', {}).get('score')}) [Code: {teams.get('away', {}).get('code')}]"
            )
            print(
                f"Home: {teams.get('home', {}).get('name')} ({teams.get('home', {}).get('score')}) [Code: {teams.get('home', {}).get('code')}]"
            )
        else:
