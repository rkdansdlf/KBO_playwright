        ...
        try:
            ...
            for row in session.query(GameBattingStat).filter_by(game_id=game.game_id).all():
                if not row.player_id:
                    continue
                existing_batting_exact[(row.team_side, row.player_name, row.batting_order, row.appearance_seq)] = (
                    row.player_id
                )
            ...
            for row in session.query(GamePitchingStat).filter_by(game_id=game.game_id).all():
                if not row.player_id:
                    continue
                existing_pitching_exact[(row.team_side, row.player_name, row.is_starting, row.appearance_seq)] = (
                    row.player_id
                )
            ...
            if hitters.get("away") or hitters.get("home"):
                ...
                for side in ["away", "home"]:
                    for idx, h in enumerate(hitters.get(side, []), start=1):
                        ...
                        if not player_id:
                            player_id = (
                                existing_batting_exact.get(
                                    (side, h["player_name"], h.get("batting_order"), appearance_seq)
                                )
                        ...
                        if stat.plate_appearances == 0:
                            stat.plate_appearances = (
                                (stat.at_bats or 0)
                                + (stat.walks or 0)
                                + (h["stats"].get("intentional_walks") or 0)
                                + (stat.hbp or 0)
                                + (stat.sacrifice_hits or 0)
                                + (stat.sacrifice_flies or 0)
                            )
                        session.add(stat)
            else:
