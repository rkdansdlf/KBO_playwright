    ...
    with engine.connect() as conn:
        ...
        for game in games:
            ...
            if new_home != home_team or new_away != away_team:
                conn.execute(
                    text("UPDATE game SET home_team = :home, away_team = :away WHERE id = :id"),
                ...
                for table in ["game_inning_scores", "game_batting_stats", "game_pitching_stats", "game_lineups"]:
                    conn.execute(
                        text(
                            f"UPDATE {table} SET team_code = :new WHERE game_id = (SELECT game_id FROM game WHERE id = :id) AND team_code = :old"
                        ),
