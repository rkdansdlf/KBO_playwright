...
def check_health():
    ...
    total_retired = local_session.execute(text("SELECT COUNT(*) FROM players WHERE status='RETIRED'")).scalar()

    print(f"📸 Master Records: Retired players with photo: {retired_with_photo} / {total_retired}")

    # 1. Check for missing master records
    missing_count = local_session.execute(
        text("""
    ...
    mismatch_count = local_session.execute(
        text("""
        SELECT COUNT(*) FROM player_basic pb
