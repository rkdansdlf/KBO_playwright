...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("data/modern_recovery.log"), logging.StreamHandler()],
)
...
async def get_games_to_recover(session, year: int, pool: AsyncPlaywrightPool) -> list[dict[str, str]]:
    ...
    season_starts = {2024: 20240323, 2023: 20230401, 2022: 20220402, 2021: 20210403, 2020: 20200505}
    ...
    logger.info(
        f"Schedule: {len(games)} raw -> {len(seen_ids)} unique -> {len(valid_games)} after date filter (>= {start_date})"
    )
    return valid_games
...
async def recover_season(year: int):
    ...
    try:
        ...
        for i in range(0, len(games), chunk_size):
            ...
            logger.info(
                f"Processing chunk {i // chunk_size + 1}/{(len(games) - 1) // chunk_size + 1} ({len(chunk)} games)..."
            )
