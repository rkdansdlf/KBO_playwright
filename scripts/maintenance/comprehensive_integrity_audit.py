...
def run_audit(target_db="local"):
    ...
    game_counts = session.query(
        func.substr(Game.game_id, 1, 4).label("year"), func.count(Game.game_id).label("cnt")
    ...
    batting_runs = (
        session.query(
            GameBattingStat.game_id, GameBattingStat.team_side, func.sum(GameBattingStat.runs).label("total_runs")
        )
        .group_by(GameBattingStat.game_id, GameBattingStat.team_side)
        .all()
    )

    game_scores = {
        g.game_id: {"home": g.home_score, "away": g.away_score}
    ...
    pitching_runs = (
        ...
        .group_by(GamePitchingStat.game_id, GamePitchingStat.team_side)
        .all()
    )
