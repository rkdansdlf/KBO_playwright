...
from src.models.player import PlayerBasic, Player
from sqlalchemy import func

def _delete_game_and_metadata(session, game_obj):
    """Helper to delete game and all its non-cascading children."""
    if game_obj.metadata_entry: session.delete(game_obj.metadata_entry)
    for r in game_obj.summary: session.delete(r)
    for r in game_obj.plays: session.delete(r)
    for r in game_obj.innings: session.delete(r)
    for r in game_obj.lineups: session.delete(r)
    for r in game_obj.events: session.delete(r)
    for r in game_obj.aliases: session.delete(r)
...
def cleanup_ghost_games():
    ...
    try:
        ...
        for g_date, home, away, count in duplicates:
            ...
            games = (
                session.query(Game)
                .filter_by(game_date=g_date, home_team=home, away_team=away)
                .all()
            )
            ...
            for ghost in ghosts:
                ...
                if ghost['total_stats'] == 0:
                    ...
                    if master['total_stats'] > 0:
                        print(f"   🗑️ Deleting redundant game WITH STATS {game_obj.game_id} ({g_date} {away}@{home}) - stats count {ghost['total_stats']}.")
                        
                        # Explicitly delete stats of the redundant record
                        for s in game_obj.batting_stats: session.delete(s)
                        for s in game_obj.pitching_stats: session.delete(s)
