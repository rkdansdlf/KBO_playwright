...
def main():
    ...
    with engine.connect() as conn:
        ...
        if result.rowcount > 0:
            print(f"  - Updated home_team DO -> OB: {result.rowcount} rows")

        result = conn.execute(text("UPDATE game SET away_team = 'OB' WHERE away_team = 'DO'"))
        if result.rowcount > 0:
            print(f"  - Updated away_team DO -> OB: {result.rowcount} rows")

        result = conn.execute(text("UPDATE game SET winning_team = 'OB' WHERE winning_team = 'DO'"))
        if result.rowcount > 0:
            print(f"  - Updated winning_team DO -> OB: {result.rowcount} rows")

        # 2. Update Related Tables
        tables_with_team_code = ["game_inning_scores", "game_batting_stats", "game_pitching_stats", "game_lineups"]
