...
def repair_master():
    ...
    for pb in basics:
        ...
        calc_status = "RETIRED"
        if pb.team and any(t in pb.team for t in ACTIVE_TEAMS) or pb.status == "active":
            calc_status = "ACTIVE"
        ...
        if player:
            ...
            if pb.photo_url and pb.photo_url != "NOT_FOUND":
                if (
                    not player.photo_url
                    or "no-Image.png" in player.photo_url
                    or player.photo_url == "NOT_FOUND"
                    or pb.photo_url != player.photo_url
                ):
                    player.photo_url = pb.photo_url

            # Enrich with available detailed data
            if pb.salary_original:
                player.salary_original = pb.salary_original
            if pb.signing_bonus_original:
                player.signing_bonus_original = pb.signing_bonus_original
            if pb.draft_info:
                player.draft_info = pb.draft_info
