...
def bolster_player_basic():
    with SessionLocal() as session:
        ...
        missing_ids = set()
        br_ids = session.query(PlayerSeasonBaserunning.player_id).filter(
            ~PlayerSeasonBaserunning.player_id.in_(session.query(PlayerBasic.player_id))
        ).distinct().all()
        for (pid,) in br_ids: missing_ids.add(pid)
        
        fld_ids = session.query(PlayerSeasonFielding.player_id).filter(
            ~PlayerSeasonFielding.player_id.in_(session.query(PlayerBasic.player_id))
        ).distinct().all()
        for (pid,) in fld_ids: missing_ids.add(pid)
        ...
        for pid in missing_ids:
            ...
            if bat:
                ...
                pit = session.query(GamePitchingStat.player_name, GamePitchingStat.team_code).filter_by(player_id=pid).first()
                if pit:
                    name, team = pit
