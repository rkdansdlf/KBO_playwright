...
Features:
  ...
  3. Compresses the backup with gzip for 75-85% space savings.
  4. Automatically cleans up old backups, keeping only the N most recent backups (default: 7).

Usage:
    python -m scripts.maintenance.backup_db
    python -m scripts.maintenance.backup_db --keep-count 10 --dry-run
    python -m scripts.maintenance.backup_db --no-compress
"""
...
def run_backup(
    *,
    ...
    if dry_run:
        ...
        logger.info("[DRY-RUN] Would create backup at: %s%s", target, suffix)
    ...
    try:
        ...
        # Gzip compression (75-85% space savings on typical SQLite databases)
        final_path = target
        if compress:
            gz_path = target.with_suffix(".db.gz")
            logger.info("🗜️ Compressing backup with gzip...")
            with target.open("rb") as f_in, gzip.open(gz_path, "wb", compresslevel=6) as f_out:
                shutil.copyfileobj(f_in, f_out)
            gz_size_mb = gz_path.stat().st_size / (1024 * 1024)
            ratio = (1 - gz_size_mb / size_mb) * 100 if size_mb > 0 else 0
            logger.info(
                "✅ Compressed: %.2f MB → %.2f MB (%.1f%% reduction)",
                size_mb,
                gz_size_mb,
                ratio,
            )
            target.unlink()  # Remove uncompressed file
            final_path = gz_path
...
def _rotate_backups(backup_dir: Path, keep_count: int) -> None:
    # Collect both uncompressed (.db) and compressed (.db.gz) backups
    backups = sorted(
        [*backup_dir.glob("kbo_dev_*.db"), *backup_dir.glob("kbo_dev_*.db.gz")],
