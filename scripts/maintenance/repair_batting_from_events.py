    ...
    with SessionLocal() as session:
        ...
        for gid in game_ids:
            ...
            events = session.execute(
                text("SELECT batter_id, result_code FROM game_events WHERE game_id = :gid AND event_type = 'batting'"),
                {"gid": gid}
            ).mappings().all()
            ...
            for bid, s in batter_stats.items():
                ...
                if dry_run:
                    print(f"     [DRY RUN] Batter {bid}: PA={s['pa']}, SO={s['so']}, BB={s['bb']}, SH={s['sh']}, SF={s['sf']}")
                else:
                    session.execute(
                        text("""
                        ...
                        """),
                        {
                            "pa": s["pa"], "so": s["so"], "bb": s["bb"], "hbp": s["hbp"],
                            "sh": s["sh"], "sf": s["sf"], "gid": gid, "bid": bid
                        }
                    )
