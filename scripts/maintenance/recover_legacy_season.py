...
def recover_legacy_season(year: int, save: bool = False, headless: bool = True):
    ...
    batting_data = crawl_legacy_batting_stats(year=year, series_key="regular", save_to_db=save, headless=headless)
    print(f"✅ Recovered {len(batting_data)} batting records.")

    # 2. Recover Pitching Stats
    print("\n[PITCHING] Crawling legacy pitching stats...")
    pitching_data = crawl_legacy_pitching_stats(year=year, series_key="regular", save_to_db=save, headless=headless)
