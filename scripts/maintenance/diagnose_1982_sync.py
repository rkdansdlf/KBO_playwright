...
def diagnose_1982():
    ...
    with Engine.connect() as local_conn:
        local_ids = set(
            ...
            for r in local_conn.execute(
                text("SELECT player_id FROM player_season_batting WHERE season=1982")
            ).fetchall()
        )
    ...
    with oci_engine.connect() as oci_conn:
        oci_ids = set(
            r[0]
            for r in oci_conn.execute(text("SELECT player_id FROM player_season_batting WHERE season=1982")).fetchall()
        )
