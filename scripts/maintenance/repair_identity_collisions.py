...
def get_player_candidates(session, name, season, team_code):
    ...
    """), {"name": name, "season": season, "team_code": team_code}).fetchall()
    
    # Check pitching
    pitchers = session.execute(text("""
...
def repair_batting_collisions():
    ...
    try:
        ...
        collisions = session.execute(text("""
        ...
        for game_id, current_player_id, name, count in collisions:
            ...
            """), {"game_id": game_id, "current_player_id": current_player_id}).fetchall()
            
            team_code = rows[0].team_code
            candidates = get_player_candidates(session, name, season, team_code)
            
            if len(candidates) <= 1:
                logger.warning(f"  Game {game_id}: {name} has {len(candidates)} candidates in season data. Cannot disambiguate easily.")
                continue
                
            logger.info(f"  Processing Game {game_id}: {name} ({len(candidates)} candidates: {list(candidates.keys())})")
            ...
            for row in rows:
                ...
                if len(possible_ids) == 1:
                    ...
                    if target_id != current_player_id:
                        logger.info(f"    -> Updating row {row.id} (pos {row.position}): {current_player_id} -> {target_id}")
                        session.execute(text("UPDATE game_batting_stats SET player_id = :target_id WHERE id = :id"), 
                                        {"target_id": target_id, "id": row.id})
                        repaired_count += 1
                else:
                    logger.warning(f"    Could not uniquely disambiguate row {row.id} (pos {row.position}). Candidates: {possible_ids}")

        session.commit()
        logger.info(f"Repaired {repaired_count} batting rows.")
        
        # 2. Game Lineups
        lineup_collisions = session.execute(text("""
            SELECT game_id, player_id, player_name, COUNT(*) as cnt 
            FROM game_lineups 
            GROUP BY game_id, player_id 
            HAVING cnt > 1
        ...
        for game_id, current_player_id, name, count in lineup_collisions:
            ...
            for row in rows:
                ...
                if len(possible_ids) == 1:
                    target_id = possible_ids[0]
                    if target_id != current_player_id:
                        session.execute(text("UPDATE game_lineups SET player_id = :target_id WHERE id = :id"), 
                                        {"target_id": target_id, "id": row.id})
