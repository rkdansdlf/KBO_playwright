...
def _migration_files(dialect: str, migration_dir: str | None = None) -> list[Path]:
...
def check_migrations(dialect: str = "postgres", migration_dir: str | None = None) -> int:
    ...
    local_paths = _migration_files(dialect, migration_dir)
...
def apply_migrations(dialect: str = "postgres", migration_dir: str | None = None) -> int:
    load_dotenv()
    oci_url = _get_oci_url()
    if not oci_url:
        print("❌ OCI_DB_URL is not set.")
    ...
    engine = create_engine_for_url(oci_url)
    Session = sessionmaker(bind=engine)
    migration_files = _migration_files(dialect, migration_dir)
    table_name = MIGRATION_TABLES[dialect]
...
if __name__ == "__main__":
    ...
    if args.check:
        sys.exit(check_migrations(args.dialect, args.migration_dir))
    else:
        sys.exit(apply_migrations(args.dialect, args.migration_dir))
