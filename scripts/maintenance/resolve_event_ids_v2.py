...
def resolve_ids_v2():
    with SessionLocal() as session:
        ...
        games = (
            session.query(Game.game_id, Game.home_team, Game.away_team, Game.home_pitcher, Game.away_pitcher)
            .join(GameEvent, Game.game_id == GameEvent.game_id)
            .distinct()
            .all()
        )
        ...
        for g_id, home_team, away_team, home_starter, away_starter in games:
            ...
            pit_map = {
                row.player_name: row.player_id for row in session.query(GamePitchingStat).filter_by(game_id=g_id).all()
            }
