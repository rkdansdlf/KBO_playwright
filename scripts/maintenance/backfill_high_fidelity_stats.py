...
def get_opponent_code(name: str) -> Optional[str]:
    return TEAM_NAME_TO_CODE.get(name)

async def backfill_batting_stats(year: int, player_ids: Optional[List[int]] = None):
    ...
    try:
        ...
        for p_id, p_name in players:
            ...
            for game in games:
                ...
                db_game_ids = session.execute(text("""
                    SELECT game_id FROM game 
                    WHERE game_date = :gdate 
                    AND (home_team = :opp OR away_team = :opp)
                    ORDER BY game_id ASC
                """), {'gdate': g_date, 'opp': opp_code}).fetchall()
                ...
                """), {
                    'pa': stats['plate_appearances'], 'hr': stats['home_runs'], 'bb': stats['walks'], 'hbp': stats['hbp'],
                    'so': stats['strikeouts'], 'gdp': stats['gdp'], 'd2': stats['doubles'], 'd3': stats['triples'],
                    'r': stats['runs'], 'rbi': stats['rbi'], 'sb': stats['stolen_bases'], 'cs': stats['caught_stealing'],
                    'pid': p_id, 'gid': target_gid
                })
                update_count += 1
...
async def backfill_pitching_stats(year: int, player_ids: Optional[List[int]] = None):
        ...
        for p_id, p_name in players:
            ...
            for game in games:
                ...
                session.execute(text(f"""
                    ...
                    WHERE player_id = :pid AND game_id = :gid
                """), {
                    'dec': stats['decision'], 'w': stats['wins'], 'l': stats['losses'], 's': stats['saves'],
                    'tbf': stats['batters_faced'], 'outs': stats['innings_outs'], 'h': stats['hits_allowed'],
                    'hr': stats['home_runs_allowed'], 'bb': stats['walks_allowed'], 'hbp': stats['hbp_allowed'],
                    'so': stats['strikeouts'], 'r': stats['runs_allowed'], 'er': stats['earned_runs'],
                    'pid': p_id, 'gid': target_gid
                })
                update_count += 1
