...
- RAG source inventory is structurally clean (`17` sources, `207,259` generated chunks, zero duplicate/invalid identities and missing metadata). The canonical production index remains untouched; isolated staging sparse and configured-vector indexes both contain `207,259` rows with zero orphan, vector-only, hash/version mismatch, missing-embedding, or stale rows and `consistent=true`. The configured vector index uses OpenRouter `perplexity/pplx-embed-v1-4b` at 1536 dimensions with a valid cosine HNSW index. `RAG_EMBED_BATCH_SIZE=50` and rate-limit retry/backoff completed the full build; transient 429 responses recovered without data loss. The 30-query configured golden draft reports BM25 Recall@5 `0.3333` / MRR `0.2833`, vector `0.9333` / `0.8056`, hybrid `0.9333` / `0.8056`, and resolver-hybrid `0.9333` / `0.8222`; retrieval p95 is approximately `467ms`, `351ms`, `396ms`, and `397ms`. Routing separately reports `100/100` intent, route, and entity accuracy with zero false positives. Metrics are staging evidence pending final golden-label, threshold, cost, and production-promotion approval.
