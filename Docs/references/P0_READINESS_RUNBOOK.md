...
P0 backfill처럼 이미 대상 경기 데이터를 보강했고 summary만 확인해야 하는 scoped 실행에서는 비-P0 병목을 명시적으로 제외할 수 있다.

```bash
python3 -m src.cli.run_daily_update --date YYYYMMDD --skip-auto-healer --skip-season-stats --skip-p0-non-game
```
...
이 env는 `run_daily_update`에 `--skip-season-stats`를 전달한다. full finalize로 복귀하려면 unset 또는 `0`으로 둔다.
