...
- **Post-fix automation (follow-up)**: `crawl_p1p2_data_job` now writes a run marker `data/last_runs/p1p2_data.json` (ts + status ok/error) on every run; `scripts/check_p1p2_lock_health.py` reads it (replacing the fragile log-grep) and scans only the last 8 MB of `logs/scheduler.launchd.err.log` (the real operational log) for forbidden lock-error signatures. A lock-free `lock_health_check` job runs daily at 06:50 KST, invokes the check with `--require-run`, and alerts via `alert_warning` (Telegram/Slack) on failure. Tests: `tests/scripts/test_check_p1p2_lock_health.py`, `tests/scripts/test_scheduler_lock_health_job.py`.
