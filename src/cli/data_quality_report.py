    ...
    with SessionFactory() as session:
        for year in years:
            ...
            batting_stats = session.query(
                PlayerSeasonBatting.source, func.count(PlayerSeasonBatting.id)
            ).filter(PlayerSeasonBatting.season == year).group_by(PlayerSeasonBatting.source).all()
            ...
            pitching_stats = session.query(
                PlayerSeasonPitching.source, func.count(PlayerSeasonPitching.id)
            ).filter(PlayerSeasonPitching.season == year).group_by(PlayerSeasonPitching.source).all()
            ...
            # 3. Fielding Summary
            fielding_stats_query = session.query(
                PlayerSeasonFielding.source, func.count(PlayerSeasonFielding.id)
            ).filter(PlayerSeasonFielding.year == year).group_by(PlayerSeasonFielding.source).all()
            ...
            baserunning_stats_query = session.query(
                PlayerSeasonBaserunning.source, func.count(PlayerSeasonBaserunning.id)
            ).filter(PlayerSeasonBaserunning.year == year).group_by(PlayerSeasonBaserunning.source).all()
        ...
        with open(path, "w", encoding="utf-8", newline="") as f:
            ...
            for yr, data in report_data["years"].items():
                for cat in ["batting", "pitching", "fielding", "baserunning"]:
                    ...
                    writer.writerow([yr, cat.upper(), data[cat]["total"], data[cat].get("consistency_rate", "N/A"), sources])
        print(f"✅ Report saved to {path}")
