...
def rebuild_rankings(season: int, *, external_provider: str | None = None) -> int:
    """Rebuild all stat_rankings for a season from local DB.

    Args:
        season: Season year.
        external_provider: Optional external provider source filter.
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--external-provider", type=str, default=None, help="External provider source")
    args = parser.parse_args(argv)

    kwargs = {}
    if args.external_provider is not None:
        kwargs["external_provider"] = args.external_provider

    rebuild_rankings(args.year, **kwargs)
    return 0
