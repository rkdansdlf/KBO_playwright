...
class SqliteToOciSynchronizer:
    ...
    def _execute_write_batches(self, ctx: WriteBatchContext) -> tuple[int, int]:
        ...
        for offset in range(0, len(all_rows), batch_size):
            ...
            success_cnt, err_cnt = ctx.writer.execute_batch(ctx.sync_sql, payloads, ctx.table)
...
def _resolve_target_url(explicit_url: str | None) -> str | None:
    ...
    configured = explicit_url or os.getenv("ORACLE_TARGET_URL") or os.getenv("OCI_DB_URL")
    if configured:
        return configured
    database_url = os.getenv("DATABASE_URL")
