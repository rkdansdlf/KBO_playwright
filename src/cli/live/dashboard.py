...
def generate_dashboard() -> None:
    ...
    for m in metrics:
        ...
        notes_str = html_mod.escape(str(m.notes or "-"))
        category_str = html_mod.escape(str(m.category.upper() if m.category else ""))

        rows_html.append(
            f"""
            <tr>
                <td>{check_time_str}</td>
                <td><span class="badge cat-{html_mod.escape(str(m.category or ""))}">{category_str}</span></td>
