...
class PlayerSeasonBattingRepository:
    ...
    def _upsert_one(self, session: Session, stats_data: dict[str, Any]):
        ...
        if self.dialect == "sqlite":
            ...
            update_dict = {k: stmt.excluded[k] for k in data if k not in unique_keys}
            stmt = stmt.on_conflict_do_update(index_elements=unique_keys, set_=update_dict)
    ...
    def get_by_player_season(
        ...
        with SessionLocal() as session:
            return (
                ...
                .filter_by(player_id=player_id, season=season, league=league, level=level)
                .first()
            )
    ...
    def get_by_season(
        ...
        with SessionLocal() as session:
            query = (
                session.query(PlayerSeasonBatting)
                .filter_by(season=season, league=league, level=level)
