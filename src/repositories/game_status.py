...
def update_game_status(game_id: str, status: str, session: Session | None = None) -> bool:
    ...
    if session is None:
        try:
            with get_db_session() as s:
                return update_game_status(game_id, status, session=s)
        except Exception:
...
def refresh_game_status_for_date(
    target_date: str,
    ...
    if session is None:
        try:
            with get_db_session() as s:
                return refresh_game_status_for_date(target_date, today, session=s)
