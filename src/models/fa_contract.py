...
class FAContract(Base, TimestampMixin):
    ...
    contract_duration: Mapped[str | None] = mapped_column(
        String(50),
        nullable=True,
        comment="Original contract duration text (e.g. '4년')",
    )
    total_amount: Mapped[str | None] = mapped_column(
        String(50),
        nullable=True,
        comment="Original total contract amount text (e.g. '75억원')",
    )
    total_amount_krw: Mapped[int | None] = mapped_column(
        Integer,
        nullable=True,
        comment="Parsed total contract amount in 10,000 KRW (만원) units",
    )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"<FAContract(player='{self.player_name}', year={self.year}, type='{self.fa_type}', "
            f"team='{self.new_team}', amount='{self.total_amount}')>"
        )
