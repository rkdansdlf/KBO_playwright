...
class RagChunk(Base, TimestampMixin):
    ...
    league_type_code: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="League level code (e.g. KBO vs Futures)",
    )
    ...
    source_table: Mapped[str] = mapped_column(
        Text, nullable=False, comment="Source descriptor (e.g. naver_news, rulebook, namuwiki)",
    )
    source_row_id: Mapped[str] = mapped_column(
        Text, nullable=False, comment="Unique key or URL of the source article/document",
    )
    ...
    meta: Mapped[dict] = mapped_column(
        JSON, nullable=True, default=dict, server_default="{}", comment="Additional metadata mappings",
    )
