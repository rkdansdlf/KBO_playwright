...
class DataSource(Base, TimestampMixin):
    ...
    source_key: Mapped[str] = mapped_column(
        ...
        nullable=False,
        unique=True,
        comment="Unique source key (e.g. kbo_schedule, lg_ticket_general)",
    )
    ...
    crawl_frequency: Mapped[str | None] = mapped_column(
        String(30),
        nullable=True,
        comment="daily / weekly / seasonal / ondemand / event_driven",
    )
    ...
    is_active: Mapped[bool] = mapped_column(
        default=True,
        server_default="1",
        comment="Whether this source is actively monitored",
    )
...
class RawSourceSnapshot(Base, TimestampMixin):
    ...
    parser_version: Mapped[str | None] = mapped_column(
        String(30),
        nullable=True,
        comment="Parser version used for this snapshot",
    )
    reprocess_status: Mapped[str | None] = mapped_column(
        String(20),
        nullable=True,
        default=None,
        comment="pending / done / failed (for batch reprocessing)",
    )
