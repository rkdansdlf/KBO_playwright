...
class StadiumSeatSection(Base, TimestampMixin):
    ...
    section_code: Mapped[str | None] = mapped_column(
        String(50),
        nullable=True,
        comment="Section code (e.g. 101B, 1루 내야)",
    )
    ...
    seat_grade: Mapped[str | None] = mapped_column(
        String(50),
        nullable=True,
        comment="Seat grade for price matching (e.g. 블루석)",
    )
    ...
    is_home_cheering: Mapped[bool] = mapped_column(
        ...
        default=False,
        server_default="0",
        comment="Home cheering section",
    )
    is_away_cheering: Mapped[bool] = mapped_column(
        Boolean,
        default=False,
        server_default="0",
        comment="Away cheering section",
    )
    ...
    geometry_json: Mapped[dict | None] = mapped_column(
        JSON().with_variant(SQLiteText, "sqlite"),
        nullable=True,
