...
class TeamPitchingStatsCrawler(BaseCrawler):
    ...
    def _collect_from_site(self, season: int, team_mapping: dict[str, str], *, headless: bool) -> list[dict[str, Any]]:
        self._last_failure_reason = None
        allowed_urls = [url for url in TEAM_PITCHING_URLS if compliance.is_allowed_sync(url)]
        if not allowed_urls:
            self._last_failure_reason = log_source_limited("team_pitching", TEAM_PITCHING_URLS[0])
            return []
            ...
            for url in allowed_urls:
                try:
                    ...
                    if not self._select_season(page, season):
                        logger.warning("Could not select team pitching season %s on %s", season, url)
                        continue
                    ...
                    logger.warning("Incomplete team pitching table for season %s from %s", season, url)
                except TEAM_PITCHING_EXCEPTIONS:
