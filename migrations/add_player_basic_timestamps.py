...
def migrate_oci():
    ...
    with engine.connect() as conn:
        ...
        if "created_at" not in existing:
            ...
            conn.execute(
                text(
                    "ALTER TABLE player_basic ADD COLUMN created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP"
                )
            )
        ...
        if "updated_at" not in existing:
            ...
            conn.execute(
                text(
                    "ALTER TABLE player_basic ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP"
                )
            )
