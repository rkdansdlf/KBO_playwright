-- Approved Oracle port of 024_deletion_anomaly_integrity.sql.
-- The pre-apply Oracle backup and orphan census were reviewed before release.
-- Oracle DDL auto-commits; this migration is not automatically reversible.
...
BEGIN
    ...
    IF v_table_exists = 1 THEN
        SELECT COUNT(*)
          INTO v_pending
          FROM TEAM_DAILY_ROSTER roster
         WHERE roster.POSITION = '포지션'
            OR roster.PERSON_TYPE IS NULL
            OR (roster.PERSON_TYPE = 'player'
                AND (roster.PLAYER_BASIC_ID IS NULL OR roster.PLAYER_BASIC_ID <> roster.PLAYER_ID))
            OR (roster.PERSON_TYPE <> 'player' AND roster.PLAYER_BASIC_ID IS NOT NULL);
    END IF;
    IF v_table_exists = 1 AND v_pending > 0 THEN
        EXECUTE IMMEDIATE 'ALTER TABLE TEAM_DAILY_ROSTER DISABLE ALL TRIGGERS';
        UPDATE /*+ NO_PARALLEL */ TEAM_DAILY_ROSTER roster
         ...
         UPDATE /*+ NO_PARALLEL */ TEAM_DAILY_ROSTER roster
            SET roster.PERSON_TYPE = CASE
               ...
               ELSE 'unknown'
            END
          WHERE roster.PERSON_TYPE IS NULL
             OR roster.PERSON_TYPE <> CASE
                WHEN roster.POSITION IN ('투수', '포수', '내야수', '외야수', '선수') THEN 'player'
                WHEN roster.POSITION IN ('감독', '코치') THEN 'staff'
                ELSE 'unknown'
            END;
         ...
         UPDATE /*+ NO_PARALLEL */ TEAM_DAILY_ROSTER roster
          ...
          WHERE roster.PERSON_TYPE <> 'player'
            AND roster.PLAYER_BASIC_ID IS NOT NULL;
...
UPDATE /*+ NO_PARALLEL */ PLAYER_MOVEMENTS pm
   ...
   END
 WHERE pm.CANONICAL_TEAM_ID IS NULL;
/
...
-- Unique-name identity resolution is intentionally delegated to the reviewed
-- 025-028 backfill sequence to avoid a self-update deadlock on Oracle RAC.
BEGIN
    NULL;
END;
/

-- Resolution status is already maintained by the source sync and the
-- reviewed 025-028 sequence. Avoid a full-table no-op update on Oracle RAC.
BEGIN
    NULL;
END;
/
