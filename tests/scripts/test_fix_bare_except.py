...
class TestFixBareExcept:
    ...
    def test_fix_file_no_change(self):
        ...
        with patch.object(Path, "read_text", return_value="x = 1"), patch.object(Path, "write_text") as mock_write:
    ...
    def test_fix_file_with_pattern(self):
        ...
        source = "try:\n    pass\nexcept Exception:\n    pass\n"
        with patch.object(Path, "read_text", return_value=source), patch.object(Path, "write_text") as mock_write:
            result = fix_file(Path("dummy.py"))
            assert result == 1
