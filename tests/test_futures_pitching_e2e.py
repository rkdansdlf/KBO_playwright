...
async def main():
    print(f"=== Futures Pitching E2E Test ===\n")
    ...
    # Show sample
    for row in rows[:3]:
        print(f"  {row.get('season')}: ERA={row.get('era')}, G={row.get('games')}, W={row.get('wins')}, L={row.get('losses')}, IP={row.get('innings_pitched')}")
    print()
    ...
    player = repo.upsert_player_profile(
        PLAYER_ID,
        PlayerProfileParsed(is_active=True, player_name="소형준")
    )
    ...
    with SessionLocal() as session:
        stmt = select(PlayerSeasonPitching).where(
            PlayerSeasonPitching.player_id == player.player_basic_id,
        ...
        for record in results:
            print(f"  {record.season}: ERA={record.era}, G={record.games}, W={record.wins}, L={record.losses}, IP={record.innings_pitched}")
