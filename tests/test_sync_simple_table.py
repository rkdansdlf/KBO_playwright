...
def _spy_bulk(syncer):
    spy = {}
    syncer._bulk_copy_upsert = lambda tbl, options: spy.update(tbl=tbl, options=options)
    return spy
...
class TestSyncSimpleTable:
    def test_syncs_all_records(self, syncer, session):
        ...
        result = syncer.sync_simple_table(_SampleModel, SimpleTableSyncOptions(conflict_keys=["name"]))
        assert result == 2
        assert spy["tbl"] == "sample_table"
        assert spy["options"].unique_cols == ["name"]
        assert len(spy["options"].records) == 2

    def test_returns_0_when_table_missing(self, syncer, session):
        _target_exists(syncer, False)
        assert syncer.sync_simple_table(_SampleModel, SimpleTableSyncOptions(conflict_keys=["name"])) == 0
    ...
    def test_excludes_id_by_default(self, syncer, session):
        ...
        spy = _spy_bulk(syncer)
        _target_exists(syncer)
        syncer.sync_simple_table(_SampleModel, SimpleTableSyncOptions(conflict_keys=["name"]))
        assert "id" not in spy["options"].records[0]
    ...
    def test_exclude_cols_extended(self, syncer, session):
        ...
        syncer.sync_simple_table(
            _SampleModel,
            SimpleTableSyncOptions(conflict_keys=["name"], exclude_cols=["value"]),
        )
        record = spy["options"].records[0]
    ...
    def test_exclude_cols_auto_adds_id(self, syncer, session):
        ...
        spy = _spy_bulk(syncer)
        _target_exists(syncer)
        syncer.sync_simple_table(_SampleModel, SimpleTableSyncOptions(conflict_keys=["name"], exclude_cols=[]))
        assert "id" not in spy["options"].records[0]
    ...
    def test_applies_filters(self, syncer, session):
        ...
        syncer.sync_simple_table(
            _SampleModel,
            SimpleTableSyncOptions(conflict_keys=["name"], filters=[_SampleModel.value == 1]),
        )
        assert len(spy["options"].records) == 1
        assert spy["options"].records[0]["name"] == "keep"
    ...
    def test_null_created_at_filled(self, syncer, session):
        ...
        _target_exists(syncer)
        syncer.sync_simple_table(_SampleModel, SimpleTableSyncOptions(conflict_keys=["name"]))
        assert isinstance(spy["options"].records[0]["created_at"], datetime)
    ...
    def test_existing_created_at_preserved(self, syncer, session):
        ...
        _target_exists(syncer)
        syncer.sync_simple_table(_SampleModel, SimpleTableSyncOptions(conflict_keys=["name"]))
        assert spy["options"].records[0]["created_at"] == dt
    ...
    def test_null_updated_at_filled(self, syncer, session):
        ...
        _target_exists(syncer)
        syncer.sync_simple_table(_SampleModel, SimpleTableSyncOptions(conflict_keys=["name"]))
        assert isinstance(spy["options"].records[0]["updated_at"], datetime)
    ...
    def test_transform_fn_applied(self, syncer, session):
        ...
        syncer.sync_simple_table(
            _SampleModel,
            SimpleTableSyncOptions(conflict_keys=["name"], transform_fn=upper_name),
        )
        assert spy["options"].records[0]["name"] == "HELLO"
    ...
    def test_transform_fn_adding_keys(self, syncer, session):
        ...
        syncer.sync_simple_table(
            _SampleModel,
            SimpleTableSyncOptions(conflict_keys=["name"], transform_fn=add_key),
        )
        assert spy["options"].records[0].get("extra") == "x"
    ...
    def test_json_serialization_of_dict_objects(self, syncer, session):
        ...
        syncer.sync_simple_table(
            _SampleModel,
            SimpleTableSyncOptions(conflict_keys=["name"], transform_fn=inject_dict),
        )
        assert spy["options"].records[0]["payload"] == '{"hello": "world"}'
    ...
    def test_json_serialization_of_list_objects(self, syncer, session):
        ...
        syncer.sync_simple_table(
            _SampleModel,
            SimpleTableSyncOptions(conflict_keys=["name"], transform_fn=inject_list),
        )
        assert spy["options"].records[0]["payload"] == "[1, 2, 3]"
    ...
    def test_update_timestamp_true(self, syncer, session):
        ...
        syncer.sync_simple_table(
            _SampleModel,
            SimpleTableSyncOptions(conflict_keys=["name"], update_timestamp=True),
        )
        assert spy["options"].update_timestamp is True
    ...
    def test_update_timestamp_false(self, syncer, session):
        ...
        syncer.sync_simple_table(
            _SampleModel,
            SimpleTableSyncOptions(conflict_keys=["name"], update_timestamp=False),
        )
        assert spy["options"].update_timestamp is False
    ...
    def test_update_timestamp_default_depends_on_exclude_cols(self, syncer, session):
        ...
        syncer.sync_simple_table(
            _SampleModel,
            SimpleTableSyncOptions(conflict_keys=["name"], exclude_cols=["updated_at"]),
        )
        assert spy["options"].update_timestamp is False
    ...
    def test_batch_splits_records(self, syncer, session):
        ...
        batches = []
        _target_exists(syncer)
        syncer._bulk_copy_upsert = lambda _tbl, options: batches.append(len(options.records))
        syncer.sync_simple_table(_SampleModel, SimpleTableSyncOptions(conflict_keys=["name"], batch_size=3))
