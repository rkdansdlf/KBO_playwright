...
class TestCollectGamesCLI:
    def test_main_year_default(self):
        with patch("src.cli.collect_games.load_game_targets_from_db") as mock_load, \
            ...
            result = MagicMock(detail_saved=0, detail_failed=0, detail_skipped_existing=0,
                               relay_saved_games=0, relay_rows_saved=0, relay_skipped_existing=0,
                               processed_game_ids=set(), items={})
    ...
    def test_main_with_month(self):
        with patch("src.cli.collect_games.load_game_targets_from_db") as mock_load, \
            ...
            mock_load.return_value = []
            mock_sesh.return_value = MagicMock()
            result = MagicMock(detail_saved=3, detail_failed=0, detail_skipped_existing=0,
                               relay_saved_games=0, relay_rows_saved=0, relay_skipped_existing=0,
                               processed_game_ids=set(), items={})
            mock_crawl.return_value = result
    ...
    def test_main_with_game_ids(self):
        with patch("src.cli.collect_games.load_game_targets_by_ids") as mock_load, \
             patch("src.cli.collect_games.SessionLocal") as mock_sesh, \
             patch("src.services.player_id_resolver.PlayerIdResolver"), \
             patch("src.cli.collect_games.GameDetailCrawler"), \
             patch("src.cli.collect_games.NaverRelayCrawler"), \
