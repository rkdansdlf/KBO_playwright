...
class TestCrawlPhase1ExtraCLI:
    def test_main_broadcast(self):
        with patch("sys.argv", ["crawl_phase1_extra", "--type", "broadcast", "--save"]), \
    ...
    def test_main_mvp(self):
        with patch("sys.argv", ["crawl_phase1_extra", "--type", "mvp", "--save"]), \
    ...
    def test_main_seed_stadium(self):
        with patch("sys.argv", ["crawl_phase1_extra", "--type", "seed_stadium"]), \
             patch("src.cli.crawl_phase1_extra.seed_stadium_info") as mock_seed:
            main()
            mock_seed.assert_called_once()

    def test_main_all_crawlers(self):
        with patch("sys.argv", ["crawl_phase1_extra", "--type", "crawlers", "--save"]), \
