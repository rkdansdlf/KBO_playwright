...
def _build_repo(tmp_path):
    ...
    return PlayerBasicRepository(session), engine


def test_upsert_players_filters_invalid_payloads_before_save(tmp_path):
...
def test_upsert_players_deduplicates_by_player_id_with_last_valid_row(tmp_path):
    repo, engine = _build_repo(tmp_path)
...
def test_upsert_players_accepts_legacy_player_name_key(tmp_path):
    repo, engine = _build_repo(tmp_path)
